"""
BUS-BRA Augmentation Benchmark Pipeline v3
============================================
...
Usage:
    python busbra_pipeline_v3.py --data_root /path/to/BUSBRA --aug baseline
    python busbra_pipeline_v3.py --data_root /path/to/BUSBRA --aug modern --backbone resnet50
    python busbra_pipeline_v3.py --data_root /path/to/BUSBRA --aug combined --backbone efficientnet
...
def build_dataframe(data_root: str) -> pd.DataFrame:
    ...
    print(f"\n  BI-RADS 5 has only {(df['birads']==5).sum()} samples — "
...
def get_augmentation(aug_name: str) -> A.Compose:
    ...
    if aug_name == 'baseline':
        ...
        return A.Compose([
            A.Resize(IMG_SIZE, IMG_SIZE),
            A.GaussNoise(std_range=(0.03, 0.10), p=0.5),
            A.GaussianBlur(blur_limit=(3, 5), p=0.3),
            A.RandomShadow(num_shadows_lower=1, num_shadows_upper=2,
                           shadow_dimension=4, p=0.3),
            A.CoarseDropout(num_holes_range=(3, 5),
                            hole_height_range=(16, 22),
                            hole_width_range=(16, 22), p=0.2),
            A.Normalize(mean=[0.485, 0.456, 0.406],
                        std=[0.229, 0.224, 0.225]),
            ToTensorV2()
        ])
        ...
        return A.Compose([
            ...
            A.CoarseDropout(num_holes_range=(6, 10),
                            hole_height_range=(12, 18),
                            hole_width_range=(12, 18), p=0.3),
            A.Normalize(mean=[0.485, 0.456, 0.406],
                        std=[0.229, 0.224, 0.225]),
            ToTensorV2()
        ])
        ...
        return A.Compose([
            ...
            A.CoarseDropout(num_holes_range=(3, 5),
                            hole_height_range=(12, 16),
                            hole_width_range=(12, 16), p=0.2),
            A.Normalize(mean=[0.485, 0.456, 0.406],
                        std=[0.229, 0.224, 0.225]),
            ToTensorV2()
        ])
...
# ═════════════════════════════════════════════════════════════════════════════
# 5. MODEL — supports EfficientNet-B3 and ResNet-50
# ═════════════════════════════════════════════════════════════════════════════

def build_model(backbone: str = 'efficientnet') -> nn.Module:
    if backbone == 'efficientnet':
        ...
        raise ValueError(f"Unknown backbone: {backbone}. "
                         f"Choose from: efficientnet, resnet50")
    return model
...
def run_cv(df: pd.DataFrame, aug_name: str, args) -> pd.DataFrame:
    ...
    for fold_num in sorted(df['fold'].unique()):
        ...
        weights_per_cls = 1.0 / class_counts.values
        sample_weights  = train_df['label'].map(
            dict(enumerate(weights_per_cls))).values
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_m['macro_f1'] > best_f1:
                ...
                torch.save(model.state_dict(),
                           f"best_fold{fold_num}_{aug_name}_{args.backbone}.pt")
            else:
                patience_counter += 1
        ...
        for i, cat in enumerate([2, 3, 4, 5]):
            print(f"    BI-RADS {cat}: {best_metrics[f'f1_birads{cat}']:.4f}")
        print(f"  Confusion matrix (rows=true, cols=pred, labels=2,3,4,5):")
        print(f"  {cm}")
    ...
    print(f"\n{'═'*60}")
    print(f"FINAL RESULTS  |  Aug: {aug_name}  |  Backbone: {args.backbone}")
    print(f"{'─'*60}")
    for metric, label in [
        ('accuracy',   'Accuracy '),
        ('macro_f1',   'Macro-F1 '),
    ...
    out = f"results_{aug_name}_{args.backbone}.csv"
...
def parse_args():
    p = argparse.ArgumentParser(
        description='BUS-BRA Augmentation Benchmark v3')
    p.add_argument('--data_root',  type=str, required=True,
                   help='Path to BUS-BRA root folder')
...
if __name__ == '__main__':
    args = parse_args()
    print("="*60)
    print("BUS-BRA AUGMENTATION BENCHMARK v3")
    print(f"  aug={args.aug} | backbone={args.backbone} | "
